...
func NewRequestToServer(jwt_token string) *http.Response {
	...
	request, err := http.NewRequest("POST", fmt.Sprintf("http://%s:%s/internal/task", ServerHost, ServerIP), bytes.NewBuffer(jsonStr))
...
func SendResultToServer(id, result int) {
	...
	response, _ := client.Post(fmt.Sprintf("http://%s:%s/internal/task", ServerHost, ServerIP), "application/json", body)
	response.Body.Close()
